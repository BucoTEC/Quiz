...
public class QuestionService(IUnitOfWork unitOfWork) : IQuestionService
{
    ...
    public async Task<QuestionResponseDto> UpdateQuestion(Guid id, UpdateQuestionDto updateQuestionDto)
    {
        ...
        if (existingQuestionWithSameName != null && existingQuestionWithSameName.Id != existingQuestion.Id) throw new BadRequestException($"Question with same text already exists, existing question id {existingQuestionWithSameName.Id}");
